...
{ 
	...
	{ 
		...
		while (nums[--high] > comp) //--high，不能用high++否则会少计算一个元素 
			if (high == left) 
				break; if (low >= high)//两个完成交叉则停止 break; Swap(nums, low, high);//low停在比comp值大的元素上，high停在比comp值小的元素上，两者交换。 } Swap(nums, left, high); //！！！切记要换high 因为low和high可能刚好相等，但也可能完成交错，使得low=9,high=8之类的。 return high; }
```